...
$title = 'Edit Data Pegawai | Admin SIKAMU';
$id = $_GET['id'];
		...
		<div class="col-md-12">
			<form action="aksi_pegawai.php" method="post">
				...
				<div class="form-row">
					...
					<div class="form-group col-md-4">
						<label for="">NIP</label>
						<input type="text" class="form-control" name="nip" required="" value="<?= $d['nip']?>">
					</div>
					<div class="form-group col-md-4">
						<label for="">Jabatan</label>
						<input type="text" class="form-control" name="jabatan" required="" value="<?= $d['jabatan']?>">
					</div>
				</div>
				<div class="form-row">
					<div class="form-group col-md-4">
						<label for="">Pangkat</label>
						<input type="text" class="form-control" name="pangkat" required="" value="<?= $d['pangkat']?>">
					</div>
					<div class="form-group col-md-4">
					...
					<div class="form-group col-md-4">
						...
						<button class="btn btn-danger btn-sm <?= $d['foto'] == '' ? 'd-none' : '';?>" type="button" id="reset_ttd"><i class="fa fa-redo"></i> Reset TTD</button>
						<input type="hidden" name="ttd" id="generated_ttd" value="<?= convert_img_to_base64('../uploads/ttd/'.$d['foto'])?>">	
...
<script>
	...
	$(function() {
		...
		$('#reset_ttd').click(function(){
			...
			$.ajax({
				url:'ajax_reset_ttd.php?id=<?= $d['id']?>',
